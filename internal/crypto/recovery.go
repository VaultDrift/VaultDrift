// Package crypto provides BIP39-style mnemonic recovery keys.
package crypto
...
// BIP39EnglishWordList is the 2048-word English word list for BIP39 mnemonics.
// This is embedded directly to avoid external dependencies.
var BIP39EnglishWordList = []string{
	...
	"breeze", "brick", "bridge", "brief", "bright", "brilliant", "bring", "british", "broccoli", "broken",
	...
	"comfort", "comic", "common", "company", "concert", "conduct", "confirm", "connect", "control", "cook",
	...
	"grade", "gradual", "grain", "grand", "grape", "grass", "gravity", "great", "green", "grid",
	...
	"loan", "lobster", "local", "location", "lock", "locust", "lodge", "log", "lonely", "long",
	...
	"marble", "march", "margin", "marine", "market", "marriage", "marsh", "master", "match", "material",
	"math", "matrix", "matter", "maximum", "meadow", "mean", "measure", "meat", "mechanic", "medal",
	...
	"mind", "minimum", "minister", "minor", "minute", "miracle", "mirror", "misery", "miss", "mistake",
	...
	"nest", "net", "network", "neutral", "never", "news", "next", "nice", "night", "nine",
	"noble", "node", "noise", "nominee", "noodle", "normal", "north", "nose", "notable", "note",
	...
	"shallow", "share", "shark", "sharp", "shed", "shell", "sheriff", "shield", "shift", "shine",
	...
	"steak", "steel", "stem", "step", "stereo", "stern", "stick", "still", "sting", "stock",
...
// GenerateRecoveryKey generates a 24-word recovery key from a 256-bit master key.
// The master key is combined with a checksum to create 264 bits (24 words * 11 bits).
func GenerateRecoveryKey(masterKey []byte) (string, error) {
	if len(masterKey) != 32 {
		return "", fmt.Errorf("master key must be 32 bytes, got %d", len(masterKey))
	}

	// Calculate checksum (first 8 bits of SHA-256 hash)
	checksum := sha256.Sum256(masterKey)
	checksumBits := checksum[0] >> (8 - 8) // 8 checksum bits

	// Combine: 256 bits data + 8 bits checksum = 264 bits
	// 264 bits / 11 bits per word = 24 words
	data := make([]byte, 33)
	copy(data, masterKey)
	data[32] = checksumBits
	...
	for _, idx := range indices {
		if idx < 0 || idx >= len(BIP39EnglishWordList) {
			return "", fmt.Errorf("invalid word index: %d", idx)
		}
...
// RecoverMasterKey recovers a master key from a 24-word mnemonic.
func RecoverMasterKey(mnemonic string) ([]byte, error) {
	words := strings.Fields(strings.ToLower(strings.TrimSpace(mnemonic)))
	if len(words) != 24 {
		return nil, fmt.Errorf("expected 24 words, got %d: %w", len(words), ErrInvalidMnemonic)
	}

	// Build word lookup map for efficiency
	wordMap := make(map[string]int, len(BIP39EnglishWordList))
	...
	// Convert 11-bit indices back to bytes
	data := make([]byte, 33)
	for i, idx := range indices {
		insert11Bits(data, i*11, idx)
	}

	// Extract master key (first 32 bytes)
	masterKey := make([]byte, 32)
	copy(masterKey, data[:32])

	// Verify checksum
	checksum := sha256.Sum256(masterKey)
	expectedChecksumBits := checksum[0] >> (8 - 8)
	actualChecksumBits := data[32]

	if expectedChecksumBits != actualChecksumBits {
		return nil, fmt.Errorf("checksum mismatch: %w", ErrInvalidMnemonic)
	}

	return masterKey, nil
}

// extract11Bits extracts 11 bits starting from the specified bit position.
func extract11Bits(data []byte, bitPos int) int {
	bytePos := bitPos / 8
	bitOffset := bitPos % 8

	// Need bits that may span 2-3 bytes
	var val uint32
	if bytePos < len(data) {
		val = uint32(data[bytePos]) << 24
	}
	if bytePos+1 < len(data) {
		val |= uint32(data[bytePos+1]) << 16
	}
	if bytePos+2 < len(data) {
		val |= uint32(data[bytePos+2]) << 8
	}

	// Shift to get the 11 bits we want
	val >>= (32 - 11 - bitOffset)
	return int(val & 0x7FF) // 11 bits mask
}

// insert11Bits inserts an 11-bit value at the specified bit position.
func insert11Bits(data []byte, bitPos int, val int) {
	bytePos := bitPos / 8
	bitOffset := bitPos % 8
...
// ValidateMnemonic checks if a mnemonic is valid without recovering the key.
func ValidateMnemonic(mnemonic string) bool {
	_, err := RecoverMasterKey(mnemonic)
	return err == nil
}

// GenerateRecoveryKeyWithEntropy generates a recovery key from fresh entropy.
// Returns the mnemonic and the generated master key.
func GenerateRecoveryKeyWithEntropy() (string, []byte, error) {
	masterKey, err := GenerateKey()
	if err != nil {
		return "", nil, err
	}

	mnemonic, err := GenerateRecoveryKey(masterKey)
	if err != nil {
		return "", nil, err
	}

	return mnemonic, masterKey, nil
}
